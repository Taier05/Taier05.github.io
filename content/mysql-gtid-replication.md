# MySQL GTID主从复制部署与故障处理手册

> 高风险提示：本文以 MySQL 8.0 为主。8.0.23 及以后优先使用 SOURCE/REPLICA 语法。RESET MASTER、RESET REPLICA ALL、跳过 GTID 和删除数据库都会改变复制历史或数据，执行前必须确认节点角色、保存 gtid_executed 与复制状态并完成可恢复备份。不要把“跳过错误”当作长期修复。
...
```sql
CREATE USER '<REPLICATION_USER>'@'<REPLICA_CIDR>' IDENTIFIED BY '<REPLICATION_PASSWORD>';
GRANT REPLICATION SLAVE ON *.* TO '<REPLICATION_USER>'@'<REPLICA_CIDR>';
FLUSH PRIVILEGES;
```
...
- `RESET MASTER`、`RESET REPLICA`、`RESET REPLICA ALL` 都属于高影响操作，执行前需确认角色和后果。
...
1. `SHOW REPLICA STATUS\G;` 或 `SHOW REPLICA STATUS\G;`
...
- [MySQL RESET REPLICA](https://dev.mysql.com/doc/refman/8.0/en/reset-replica.html)
